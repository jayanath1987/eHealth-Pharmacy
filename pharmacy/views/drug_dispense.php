...
<div class="container" style="width:95%;">
    <div class="row" style="margin-top:55px;">
        ...
        <div class="col-md-10 " >
            ...
            <div class="panel panel-default  "  style="padding:2px;margin-bottom:1px;" >
                <div class="panel-heading" ><b>
                    ...
                    <?php
                    if (!empty($stock_info)) {
                        echo '<b class="label label-info">' . $stock_info["name"] . ' stock in use</b>';
                    }
                    ?>
                </div>
                ...
                <div class="" style="margin-bottom:1px;padding-top:8px;">
                    ...
                    if (isset($prescribe_items_list)) {
                        if (($opd_presciption_info["Status"] == "Pending") || ($opd_presciption_info["Status"] == "Serving")) {
                        ...
                        echo '<table class="table table-condensed table-hover" style="margin-bottom:0px;">';
                        echo '<tr style="background:#e2e2e2;"><th>#</th><th>Name</th><th>Formulation</th><th>Dose</th><th>Frequency</th><th>Dose Comment</th><th>Period</th>';
                        ...
                        for ($i = 0; $i < count($prescribe_items_list); ++$i) {
                            ...
                            echo '</td>';
                            echo '<td>';
                            if (($prescribe_items_list[$i]["Frequency"]) && (($prescribe_items_list[$i]["drug_formulation"] == "Tablet") || ($prescribe_items_list[$i]["drug_formulation"] == "Capsule") || ($prescribe_items_list[$i]["drug_formulation"] == "Liquid"))) {
                                //if (($prescribe_items_list[$i]["Frequency"]) && ($prescribe_items_list[$i]["drug_formulation"] == "Tablet")){						
                                if (isset($prescribe_items_list[$i]["dose_factor"]))
                                    $d = $prescribe_items_list[$i]["dose_factor"];   // dose factor
                                if (isset($prescribe_items_list[$i]["freq_factor"]))
                                    $cnt = $prescribe_items_list[$i]["freq_factor"]; // frequency factor
                                if (isset($prescribe_items_list[$i]["period_factor"]))
                                    $n = $prescribe_items_list[$i]["period_factor"]; //Period factor

                                if (!isset($d) || $d == null)
                                    $d = 1;
                                if (!isset($cnt) || $cnt == null)
                                    $cnt = 1;
                                if (!isset($n) || $n == null)
                                    $n = 1;
                                $d_count = ceil($d * $cnt * $n);
                            }
                            else {
                                $d_count = 1;
                            }
                            if (($opd_presciption_info["Status"] == "Pending") || ($opd_presciption_info["Status"] == "Serving")) {
                                ...
                                if (isset($prescribe_items_list[$i]["drug_count"])) {
                                    //$stock_info["name"]
                                    if ($prescribe_items_list[$i]["drug_count"] > 10) {
                                        echo '&nbsp;<span class="label label-success">' . $prescribe_items_list[$i]["drug_count"] . '  in stock </span>';
                                    } else {
                                        echo '&nbsp;<span class="label label-danger">' . $prescribe_items_list[$i]["drug_count"] . '  in stock </span>';
                                    }
                                }
                            } else {
                        ...
                        echo '<center>';
                        if (($opd_presciption_info["Status"] == "Pending") || ($opd_presciption_info["Status"] == "Serving")) {
                            echo '<button type="submit" class="btn  btn-primary " autofocus>Dispense</button>&nbsp;&nbsp;';
                        }
                        ...
                        echo '<a  href="' . site_url("pharmacy") . '">Back to list</a>';
                        echo '</center>';
                        if (($opd_presciption_info["Status"] == "Pending") || ($opd_presciption_info["Status"] == "Serving")) {
                            echo '</form>';
                        }
        ...
        <div class="col-md-2">
            ...
            <!-- ALLERGY-->
            <div class="panel  panel-danger"  style="padding:2px;margin-bottom:1px;width: 250px;" >
            ...
            <div class="panel  panel-info"  style="padding:2px;margin-bottom:1px;width: 250px;" >
                <div class="panel-heading" ><b>Orders for this Visit</b></div>
                <?php
                //print_r($patient_allergy_list);
                if (isset($patient_current_lab_list) && !empty($patient_current_lab_list)) {
                    echo '<b>Lab orders</b></br>';
                    foreach ($patient_current_lab_list as $key => $value) {
                        echo $value["TestGroupName"];
                        echo '&nbsp&nbsp&nbsp';
                        echo '<span class="blink_me" style="color:red">Collection ' . $value["Collection_Status"] . '</span>';
                        echo '<br>';
                    }
                }

                if (isset($patient_current_procedure_list) && !empty($patient_current_procedure_list)) {
                    echo '<b>Procedures</b></br>';
                    foreach ($patient_current_procedure_list as $key => $value) {
                        echo $value["Treatment"];
                        echo '&nbsp&nbsp&nbsp';
                        echo '<span class="blink_me" style="color:red">' . $value["Status"] . '</span>';
                        echo '<br>';
                    }
                }

                if (isset($patient_current_injection_list) && !empty($patient_current_injection_list)) {
                    echo '<b>Injections</b></br>';
                    foreach ($patient_current_injection_list as $key => $value) {
                        echo $value["name"];
                        echo '&nbsp&nbsp&nbsp';
                        echo '<span class="blink_me" style="color:red">' . $value["status"] . '</span>';
                        echo '<br>';
            ...
            <!-- ALLERGY-->
            <div class="panel  panel-danger"  style="padding:2px;margin-bottom:1px;width: 250px;" >
                <div class="panel-heading" ><b>Other Prescription</b></div>
<?php
//print_r($patient_allergy_list);
if (isset($patient_other_prescription) && !empty($patient_other_prescription)) {
    foreach ($patient_other_prescription as $key => $value) {
        echo $value["CreateUser"];
        echo '&nbsp&nbsp&nbsp';
        
        ?>
                        <a href="<?php echo site_url("/pharmacy/dispense/" . $value["PRSID"]) . "?CONTINUE=search/sample_order"; ?>"> [ Open ]</a><br>
                        <?php
                    }
                }
                ?>
